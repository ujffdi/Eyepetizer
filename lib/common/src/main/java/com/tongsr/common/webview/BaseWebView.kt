...
open class BaseWebView @JvmOverloads constructor(
    ...
    override fun onStateChanged(source: LifecycleOwner, event: Lifecycle.Event) {
        when (event) {
            Lifecycle.Event.ON_RESUME -> {
                Timber.tag(TAG).d("onStateChanged -> ON_RESUME")
                onResume()
            }
            Lifecycle.Event.ON_STOP -> {
                Timber.tag(TAG).d("onStateChanged -> ON_STOP")
                onPause()
            }
            Lifecycle.Event.ON_DESTROY -> {
                Timber.tag(TAG).d("onStateChanged -> ON_DESTROY")
    ...
    fun interface BlankMonitorCallback {
        fun onBlank()
    }
    ...
    fun postBlankMonitorRunnable() {
        Timber.tag(TAG).d("白屏检测任务 5s 后执行")
    ...
    inner class BlankMonitorRunnable : Runnable {
        ...
        override fun run() {
            val task = Thread {
                ...
                Timber.tag(TAG).d("白屏检测任务：像素点总数为 $pixelCount")
                Timber.tag(TAG).d("白屏检测任务：白色像素点计数为 $whitePixelCount")
                Timber.tag(TAG).d("白屏检测任务：其他颜色像素点计数为 $otherPixelCount")
